...
let CreateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (checkUserExists) {
      throw new _AppError.default('This email address is already used');
    }

    const hashedPassword = await this.hashProvider.generateHash(password); // cria uma instância do usuário
