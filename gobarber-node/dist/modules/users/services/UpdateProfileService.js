...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const userEmailIsFound = await this.usersRepository.findByEmail(email);

    if (userEmailIsFound && userEmailIsFound.id !== user.id) {
      throw new _AppError.default('Duplicated email');
    }
    ...
    if (password && !old_password) {
      throw new _AppError.default('User should provide old password');
    }
    ...
    if (password && old_password) {
      ...
      if (!checkOldPassword) {
        throw new _AppError.default('User should provide correct old password');
      }
